...
const OUT_FILE = path.join(ROOT, 'src', 'data', 'contractErd.ts');
...
export const contractErdData: ContractErdData = ${JSON.stringify(data, null, 2)};
`;
}
...
async function main() {
  ...
  await fs.writeFile(OUT_FILE, renderTypeScript(data), 'utf8');

  console.log(
    `Generated ${path.relative(ROOT, OUT_FILE)} from ${REPO_URL}#${commit}: ` +
